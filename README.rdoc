...
Creating the Certificate Authority is the first order of business. You'll want to keep its keys safe etc.
...
Now say Joe loses his laptop, and we need to revoke his key:

  >> mgr.regenerate_user('joe', 'newpassword')

This will create new keys for Joe, and update the server's <tt>crl.pem</tt> revocation file. If we look up the database, we see that his <tt>active_key</tt> is now <tt>1</tt>, and <tt>0</tt> has been added to the list of revoked keys:
...
Which does the same revocation as before, but doesn't generate new keys.
